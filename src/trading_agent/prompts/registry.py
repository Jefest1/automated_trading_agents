...
PROMPTS: dict[str, PromptSpec] = {
    "supervisor": PromptSpec(
        ...
        text=(
            ...
            "1. OPEN ORDERS FIRST — NO-TOUCH + NEWS CHECK (Phase 1). Call "
            "sync_orders_from_exchange so your view is live. Filled positions are managed "
            "MECHANICALLY by the exit ladder; you never close/adjust them on price, PnL, or "
            "structure. Your only Phase-1 action: news-risk check the held symbols (via "
            "news_research) and CLOSE only on a CRITICAL adverse catalyst. Resting bids stay "
            "working until filled or TTL; cancel only on a regime flip (CANCEL_CANDIDATE in "
            "`position_reviews`) or critical news. See MANAGING OPEN ORDERS below.\n"
            ...
            "MANAGING OPEN ORDERS — NO-TOUCH: mechanical exits, news-risk is the only override\n"
            "This is a patient SPOT desk: positions are held for DAYS and exits are decided at "
            "entry, not re-litigated in-trade. Each open position is managed end-to-end by the "
            "deterministic exit_ladder: TP1 (+3%) banks 40% and moves the stop to breakeven, TP2 "
            "(+6%) banks 30% and locks the stop at TP1, TP3 (+10%) banks 15% and locks the stop "
            "at TP2, and the ~15% runner rides a trailing stop that only moves UP. TRUST IT "
            "COMPLETELY. You do NOT close, sell, or adjust a position because it is green, red, "
            ...
            "fully exit and need the order_id.\n\n"
            f"{_TOOL_REFERENCE}\n\n"
